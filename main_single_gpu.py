...
parser.add_argument('-m', '--modality', metavar='MODALITY', default='rgb', choices=["rgb", "flow"],
                    elp='modality: rgb | flow')
parser.add_argument('-d', '--dataset', default='ucf101', choices=["ucf101", "hmdb51"], help='dataset: ucf101 | hmdb51')
parser.add_argument('-a', '--arch', metavar='ARCH', default='rgb_resnet152', choices=model_names,
                    help='model architecture:  | '.join(model_names) + ' (default: rgb_vgg16)')
...
parser.add_argument('--iter-size', default=5, type=int, metavar='I',
                    help='iter size as in Caffe to reduce memory usage (default: 5)')
...
parser.add_argument('--lr_steps', default=[100, 200], type=float, nargs="+", metavar='LRSteps',
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.save_freq == 0:
            precision_1 = validate(val_loader, model, criterion)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (data, target) in enumerate(train_loader):
        ...
        precision_1, prec3 = accuracy(output.data, target, topk=(1, 3))
    ...
    for i, (data, target) in enumerate(val_loader):
        ...
        if i % args.print_freq == 0:
            print('Test: [{0}/{1}]\t'
                  ...
                  'Prec@3 {top3.val:.3f} ({top3.avg:.3f})'.format(i, len(val_loader), batch_time=batch_time, loss=losses, top1=top1, top3=top3))
